...
class TensorBoardMonitor:
    ...
    def __init__(
        ...
        self.logger.info(f"Initializing {self.__class__.__name__} ({config})")
        log_dir = (
            output_dir / "runs" / config.name if output_dir and config.name else None
        )
    ...
    def log_samples(
        ...
        """Log prompt/response samples to TensorBoard text summaries.
        ...
        for tag, problem_id in problem_ids.items():
            ...
            for sample_id in range(start_idx, start_idx + rollouts_per_problem):
                ...
                text_content = (
                    ...
                    f"Num Input Tokens: {len(input_tokens[sample_id])}\n"
                    f"Num Output Tokens: {len(output_tokens[sample_id])}"
                )
                text_name = (
                    f"samples/step_{step}/{tag}_problem_{problem_id}_sample_{sample_id}"
                )
                self.writer.add_text(text_name, text_content, global_step=step)

                # Also log scalar stats for the sample
                self.writer.add_scalar(
                    f"samples/step_{step}/{tag}_problem_{problem_id}_sample_{sample_id}/reward",
                    reward,
                    global_step=step,
                )
                self.writer.add_scalar(
                    f"samples/step_{step}/{tag}_problem_{problem_id}_sample_{sample_id}/advantage",
                    advantage,
                    global_step=step,
                )
        ...
        all_advantages = [float(a) for a in advantages]
        self.writer.add_scalar(
            "samples/mean_reward", sum(all_rewards) / len(all_rewards), global_step=step
        )
        self.writer.add_scalar(
            "samples/mean_advantage",
            sum(all_advantages) / len(all_advantages),
            global_step=step,
        )

        self.last_log_samples_step = step
        self.logger.debug(
            f"Logged samples at step {step} to TensorBoard in {time.time() - start_time:.2f}s"
        )
    ...
    def log_distributions(
        ...
        self.logger.info(
            f"Logging distributions for keys {list(distributions.keys())} to TensorBoard at step {step}"
        )
        ...
        row = {
            "step": step,
            **{k: [float(v) for v in vals] for k, vals in distributions.items()},
        }
        self.distributions.append(row)

        # Log histograms for each distribution
        for key, values in distributions.items():
            tensor_values = torch.tensor([float(v) for v in values])
            self.writer.add_histogram(
                f"distributions/{key}", tensor_values, global_step=step
            )

        # Log scalar summaries (mean, std)
        for key, values in distributions.items():
            values_float = [float(v) for v in values]
            self.writer.add_scalar(
                f"distributions/{key}/mean",
                sum(values_float) / len(values_float),
                global_step=step,
            )
            if len(values_float) > 1:
                self.writer.add_scalar(
                    f"distributions/{key}/std",
                    (
                        sum(
                            (v - sum(values_float) / len(values_float)) ** 2
        ...
        self.last_log_distributions_step = step
        self.logger.debug(
            f"Logged distributions at step {step} to TensorBoard in {time.time() - start_time:.2f}s"
        )

    def log_final_samples(self) -> None:
        """Log final samples to TensorBoard text summary."""
        if not self.is_master:
            return
        if (
            not self.config
            ...
            return
        self.logger.info("Logging final samples to TensorBoard")
        if self.samples:
            df = pd.DataFrame(self.samples)
            text_content = df.to_string(index=False)
            self.writer.add_text("final-samples/summary", text_content, global_step=0)

    def log_final_distributions(self) -> None:
        """Log final distributions to TensorBoard text summary."""
        if not self.is_master:
            return
        if (
            not self.config
            ...
            return
        self.logger.info("Logging final distributions to TensorBoard")
        if self.distributions:
            df = pd.DataFrame(self.distributions)
            text_content = df.to_string(index=False)
            self.writer.add_text(
                "final-distributions/summary", text_content, global_step=0
            )
    ...
    def save_final_summary(self, filename: str = "final_summary.json") -> None:
        ...
        dir_path = (
            self.output_dir / f"run-{int(time.time())}"
        )  # Simple timestamp-based run ID
        dir_path.mkdir(parents=True, exist_ok=True)
    ...
    def close(self) -> None:
        ...
        if self.enabled and self.is_master and hasattr(self, "writer"):
            self.writer.close()
...
def get_tb_monitor() -> TensorBoardMonitor:
    ...
    if _TB_MONITOR is None:
        raise RuntimeError(
            "TensorBoardMonitor not initialized. Please call `setup_tb_monitor` first."
        )
    return _TB_MONITOR
...
def setup_tb_monitor(
    ...
    global _TB_MONITOR
    if _TB_MONITOR is not None:
        raise RuntimeError(
            "TensorBoardMonitor already initialized. Please call `setup_tb_monitor` only once."
        )
    _TB_MONITOR = TensorBoardMonitor(
        config=config, output_dir=output_dir, tokenizer=tokenizer, run_config=run_config
    )
    return _TB_MONITOR

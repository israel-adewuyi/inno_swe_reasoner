                ...
                for idx, (prompt, cot_steps, answer) in enumerate(
                    ...
                    with autocast(dtype=torch.bfloat16):
                        ...
                        if stage == 0:
                            ...
                            target_ids = (
                                torch.tensor(input_ids.copy()[1:])
                                .unsqueeze(0)
                                .to("cuda")
                            )
                            input_ids = (
                                torch.tensor(input_ids[:-1]).unsqueeze(0).to("cuda")
                            )
                            ...
                            assert (
                                input_ids.shape == target_ids.shape == loss_mask.shape
                            ), (
                            ...
                            logits = model.embed_and_forward(
                                input_ids, position_ids, False
                            )
                            B, L, V = logits.shape
                            loss = cross_entropy(
                                logits.view(-1, V),
                            ...
                            target_ids = torch.cat(
                                [dummy_targets, target_eot_cot], dim=-1
                            )

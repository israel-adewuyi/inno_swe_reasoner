...
def setup_model(
    config: Any,
) -> AutoModelForCausalLM:  # TODO: I think model config should be the type here.
    """Returns the model specified in the config."""
    model = AutoModelForCausalLM.from_pretrained(config.name).to("cuda")
    model.gradient_checkpointing_enable()
    return CoconutModelForCausalLM(model)
